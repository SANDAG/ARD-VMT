...
SOURCE = hpms.PublicRoadDataPDFSource(
    year=2021,
    pdf_path=Path("./data/hpms/raw/prd-2021.pdf"),
    pdf_jurisdiction_page=98,
    pdf_mpo_page=146,
    extract_path=Path("./data/hpms/raw/2021 HPMS Extract.xlsx"),
...
def extract_2021prd_jurisdiction(
...
JURISDICTION_EXTRACTS = [
    ...
    for (col, key, number) in [
        ...
        ("A", "OTHER_STATE_AGENCIES", 22),
        ("A", "UNINCORPORATED", 23),
        ("A", "STATE_HIGHWAY", 24),
        ("A", "STATE_PARKS_AND_REC", 25),
        ("A", "US_FISH_AND_WILDLIFE", 26),
        ("A", "US_FOREST_SERVICE", 27),
    ]
]
